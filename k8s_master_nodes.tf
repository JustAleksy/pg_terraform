resource "yandex_compute_instance" "k8s_master_nodes" {
  count = 1
  ...
  resources {
    cores         = var.k8s_master_resources.cores
    memory        = var.k8s_master_resources.memory
    core_fraction = var.k8s_master_resources.core_fraction
  }
